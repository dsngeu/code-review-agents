...
- **Idempotency without deletion.** Re-runs (every PR push fires `synchronize`) must not duplicate. Two mechanisms, both keyed by a per-agent hidden marker (`<!-- security-review-agent -->`, `<!-- code-review-agent -->`, `<!-- branch-review-agent -->`): (1) the **summary** comment is UPDATED in place via `upsertIssueComment` (never delete-then-repost); (2) **inline** comments can't be updated as a set without deletion, so `postInline` first reads existing review comments (`fetchReviewComments`) and SKIPS any `path:line` already commented under this agent's marker. Keep the marker on every comment body — both mechanisms depend on it.
