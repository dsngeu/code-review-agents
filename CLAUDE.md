...
- **Agent 2 — Branch review** (`agents/review/branch.js`): manual (`workflow_dispatch`), reviews branch-vs-base for general quality **+** security, writes to the Actions **Job Summary**. Default model Opus.
...
- **Fail-open always.** Any error → post an error comment/Job-Summary note + `@mention`, set Check Run to `neutral`, never `failure`. Check Run conclusion is always `neutral`.
